...
describe("GET /api/v1/user", () => {
  describe("Default user", () => {
    test("With valid session", async () => {
      ...
      const cachedControl = response.headers.get("Cache-Control");

      expect(cachedControl).toBe(
    ...
    test("With nonexistent session", async () => {
      const nonexistentToken =
        "089493661fd36594400c37fcb48740e7997351694309b2e3ee6926ee60b291d914d70938c45a78c2d3d2046f5bd90723";
    ...
    test("With middle life session", async () => {
      jest.useFakeTimers({
        now: new Date(Date.now() - session.EXPIRATION_IN_MILISECONDS / 2),
      });

      const createdUser = await orchestrator.createUser({
        username: "UserWithMiddleLifeSession",
      });
